...
/**
 * Edit Estimator Option Page
 */
function bid_pdx_item_edit_page() {
    ...
    if ($item_id <= 0) {
        ?>
        <div class="wrap">
            <h1>Edit Estimator Option</h1>
            <div class="notice notice-error">
                <p>Invalid estimator option.</p>
            </div>
        </div>
        <?php
        return;
    ...
    if (!$item) {
        ...
        <div class="wrap">
            <h1>Edit Estimator Option</h1>
            <div class="notice notice-error">
                <p>Estimator option not found.</p>
            </div>
        </div>
        <?php
        return;
    ...
    if (
        ...
        wp_verify_nonce($_POST['bid_pdx_item_nonce'], 'bid_pdx_update_item_' . $item_id)
    ) {
        ...
        if ($category_id <= 0 || !$category_context) {
            ...
            $updated = Bid_PDX_Item_Manager::update(
                $item_id,
                [
            ...
            if ($updated !== false) {
                $message = 'Estimator option updated successfully.';
                $item = Bid_PDX_Item_Manager::get($item_id);
            } else {
                $error = 'The estimator option could not be updated. ' . esc_html($wpdb->last_error);
            }
    ...
    <div class="wrap">
        ...
        <p>
            <a href="<?php echo esc_url(admin_url('admin.php?page=bid-pdx-items')); ?>">
        ...
        <form method="post" action="">
            ...
            <?php wp_nonce_field('bid_pdx_update_item_' . $item_id, 'bid_pdx_item_nonce'); ?>
            ...
            <table class="form-table" role="presentation">
                        ...
                        <select name="category_id" id="category_id" required>
                            ...
                            <?php foreach ($categories as $category) : ?>
                                <option
                                    ...
                                    <?php selected(intval($item->category_id), intval($category->id)); ?>
                                >
                ...
                <tr>
                    ...
                    <td>
                        <input
                            ...
                            min="0"
                            class="regular-text"
                            value="<?php echo esc_attr($item->price_min); ?>"
                            required
                        >
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <input
                            ...
                            min="0"
                            class="regular-text"
                            value="<?php echo esc_attr($item->price_max); ?>"
                            required
                        >
                    </td>
                </tr>
                ...
                <?php
                $submitted_category_id = isset($_POST['bid_pdx_item_nonce'])
                    ? (isset($_POST['category_id']) ? intval($_POST['category_id']) : 0)
                    : intval($item->category_id);

                $submitted_condition_category_id = isset($_POST['bid_pdx_item_nonce'])
                    ? (isset($_POST['condition_category_id']) ? intval($_POST['condition_category_id']) : 0)
                    : (isset($item->condition_category_id) ? intval($item->condition_category_id) : 0);

                $submitted_condition_item_ids = isset($_POST['bid_pdx_item_nonce'])
                    ? (isset($_POST['condition_item_ids']) ? wp_unslash($_POST['condition_item_ids']) : [])
                    : (isset($item->condition_item_ids) ? $item->condition_item_ids : '');

                $submitted_condition_match = isset($_POST['bid_pdx_item_nonce'])
                    ? (isset($_POST['condition_match']) ? sanitize_key(wp_unslash($_POST['condition_match'])) : 'any')
                    : (isset($item->condition_match) ? sanitize_key($item->condition_match) : 'any');
                ...
                bid_pdx_admin_render_condition_fields([
                    ...
                    'exclude_category_id'   => $submitted_category_id,
                ]);
                ?>
                    ...
                    <td>
                        <label>
                            <input
                                ...
                                name="active"
                                value="1"
                                <?php checked(intval($item->active), 1); ?>
                            >
            ...
            <?php submit_button('Update Estimator Option'); ?>
